...
const wellKnownAuth = (
  ...
  Layer.mock(AuthWellKnown.Service)({
    all: () =>
      Effect.succeed({
        [(input.authUrl ?? "https://example.com").replace(/\/+$/, "")]: new AuthWellKnown.Entry({
          key: "TEST_TOKEN",
          token: "test-token",
        }),
      }),
    ...
    configs: () =>
      Effect.sync(() => {
        ...
        const applyToken = (value: string) => value.replace(/\{env:TEST_TOKEN\}/g, "test-token")
        const data =
          input.wellKnown ?? {
            ...(input.config !== undefined ? { config: input.config } : {}),
            ...(input.remoteConfig !== undefined ? { remote_config: input.remoteConfig } : {}),
          }
        const documents: Array<{ url: string; source: string; dir: string; content: unknown }> = []
        if (typeof data === "object" && data !== null && "config" in data && (data as { config?: unknown }).config != null) {
          documents.push({
            url: authUrl,
            source,
            dir: path.dirname(source),
            content: (data as { config?: unknown }).config,
          })
        }
        const remoteConfig =
          typeof data === "object" && data !== null && "remote_config" in data
            ? (data as { remote_config?: { url?: string; headers?: Record<string, string> } }).remote_config
            : undefined
        if (!remoteConfig?.url) return documents
        const remoteUrl = applyToken(remoteConfig.url)
        seen.remote = remoteUrl
        if (remoteConfig.headers?.Authorization) {
          seen.authorization = applyToken(remoteConfig.headers.Authorization)
        }
        documents.push({
          url: remoteUrl,
          source: remoteUrl,
...
const wellKnown = (input: {
  ...
  return {
    seen,
    it: configIt({ authWellKnown: wellKnownAuth(input, seen) }),
  }
}
