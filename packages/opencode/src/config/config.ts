  ...
  Effect.gen(function* () {
    ...
    const loadInstanceState = Effect.fn("Config.loadInstanceState")(
      function* (ctx: InstanceContext) {
        ...
        const wellKnownEntries = yield* authWellKnown.all().pipe(Effect.orDie)
        for (const entry of Object.values(wellKnownEntries)) {
...
export const node = LayerNode.make(defaultLayer, [FSUtil.node, Account.node, Env.node, Npm.node, httpClient])
