...
export const ProvidersLogoutCommand = effectCmd({
  ...
  handler: Effect.fn("Cli.providers.logout")(function* (args) {
    ...
    const selectedIndex = args.provider
      ? options.find(
          (option) =>
            credentials[option.value]?.key === args.provider ||
            database[credentials[option.value]?.key ?? ""]?.name?.toLowerCase() === args.provider?.toLowerCase(),
        )?.value
      : yield* promptValue(
          yield* Prompt.autocomplete({
            message: "Select provider",
    ...
    if (selectedIndex === undefined) return yield* fail(`Unknown configured provider "${args.provider}"`)
